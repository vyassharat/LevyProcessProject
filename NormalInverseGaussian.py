...
class NormalInverseGaussian:
    ...
    def NIG_error_function_FFT(self, p0):
        ...
        for row, option in self.options.iterrows():
            T = option["TTM"]
            model_value = self.NIG_value_call_FFT(self.S0, option['Strike Price'], T,
    ...
    def NIG_characteristic_function(self, u, x0, T, r, delta, alpha, beta, dividendRate):
        ...
        try:
            ...
            print('uh oh')
        value = np.exp((1j*u*omega*T)+(delta*T*(math.sqrt((alpha*alpha) - (beta*beta)) - math.sqrt((alpha*alpha) - ((beta+1)**2)))))
        return value
    ...
    def generate_plot(self, opt, options, singleCalibration:bool = False):
        ...
        for row, option in options.iterrows():
            ...
            options.loc[row, 'NIG Model'] = self.NIG_value_call_FFT(self.S0, option['Strike Price'],
